        ...
        if valik == "relvad":
            ...
            if valik2 == "tagasi":
                ...
                if parameeter == True and player.gold >= rüüd[valik2][0] and rüüd[valik2][1] == False:
                    ...
                    if valik2 == "nahkrüü":
                        kiiranimeeri("Nähkrüü andis 20 elupunkti juurde.")
                        player.hp += 20
                        player.hp_max += 20
                    elif valik2 == "rõngassärk":
                        kiiranimeeri("Rõngassärk andis 30 elupunkti juurde.")
                        player.hp += 30
                        player.hp_max += 30
                    elif valik2 == "raudrüü":
                        kiiranimeeri("Raudrüü andis 35 elupunkti juurde.")
                        player.hp += 35
                        player.hp_max += 35
                    elif valik2 == "meisterlik raudrüü":
                        kiiranimeeri("Meisterlik raudrüü andis 45 elupunkti juurde.")
                        player.hp += 45
                        player.hp_max += 45
                    elif valik2 == "haldjarauast rüü":
                        kiiranimeeri("NähkrüHaldjarauast rüü andis 50 elupunkti juurde.")
                        player.hp += 50
                        player.hp_max += 50
                    sleep(0.5)
...
def käsk():
    ...
    for el in "Mis on sinu järgmine tegevus?":
        ...
        time.sleep(0.025)
    tegevus = input("\n> ")
    aktsepteeritud = ["go", "mine", "liigu", "jookse", "kõnni", "jaluta", "pööra","keera", "patseeri", "flaneeri", "quit", "exit game", "quicktravel", "kiirreisi", "kiirliigu", "relvad", "käsud", "elud", "oskused", "raha", "kuld", "kulda", "võlujoogid", "rüüd"]
    ...
    while tegevus.lower() not in aktsepteeritud:
        käsk()
    if tegevus.lower() in ["go", "mine", "liigu", "jookse", "kõnni", "jaluta", "pööra", "keera", "patseeri", "flaneeri"]:
        player_movement()
    elif tegevus.lower() in ["quicktravel","kiirreisi", "kiirliigu"]:
        quicktravel = True
        player_movement()
        ...
        animeeri("Nägemiseni! :)")
        sys.exit()
    elif tegevus.lower() == "relvad":
        kiiranimeeri("Sul on relvad:\n")
        for weapon in relvad:
            if relvad[weapon][1] == True:
                print("-",weapon)
        animeeri("Kirjutage, millist relva soovite kasutada:\n")
        valik = input()
        if valik in relvad:
            if relvad[valik][1] == True:
                if valik == "vibu":
            ...
            kiiranimeeri("Tundmatu käsk, proovime uuesti...")
            käsk()
    elif tegevus.lower() == "rüüd":
        kiiranimeeri("Sul on rüüd:\n")
        for rüü in rüüd:
            if rüüd[rüü][1] == True:
                print("-",rüü)
    elif tegevus.lower() == "käsud":
        kiiranimeeri("Lubatud käsud on:\n")
        for k in aktsepteeritud:
            print("-"+k)
    elif tegevus.lower() == "võlujoogid":
        potions = []
        kiiranimeeri("Sul on võlujoogid: ")
        for jook in võlujoogid:
            if võlujoogid[jook][1] > 0:
                potions.append(jook)
                print("-" + jook + ":"+str(võlujoogid[jook][1]))
        kiiranimeeri("Kirjutage, millist jooki soovite juua:\n")
        valik = input()
        if valik in potions:
            if valik == "elujook I":
                kiiranimeeri("Jõite jooki: elujook I.\nSaite 8 elupunkti.")
                player.hp += 8
                if player.hp > player.hpmax:
                    player.hp = player.hpmax
                kiiranimeeeri("Kalevipojal on nüüd " + str(player.hp) + " elupunkti.")
            elif valik == "elujook II":
                kiiranimeeri("Jõite jooki: elujook II.\nSaite 24 elupunkti.")
                player.hp += 24
                if player.hp > player.hpmax:
                    player.hp = player.hpmax
                kiiranimeeeri("Kalevipojal on nüüd " + str(player.hp) + " elupunkti.")
            elif valik == "elujook III":
                kiiranimeeri("Jõite jooki: elujook III.\nSaite 60 elupunkti.")
                player.hp += 60
                if player.hp > player.hpmax:
                    player.hp = player.hpmax
                kiiranimeeeri("Kalevipojal on nüüd " + str(player.hp) + " elupunkti.")
            else:
                kiiranimeeri("Seda jooki saab juua vaid võitluse ajal.")
        else:
            kiiranimeeri("Tundmatu käsk, proovime uuesti...")
            käsk()
    elif tegevus.lower() == "oskused":
        kiiranimeeri("Teie praegused oskused on: ")
        for oskus in oskused:
            if oskused[oskus][1] == True:
                print("-" + oskus)
    elif tegevus.lower() == "elud":
        kiiranimeeri("Teil on " + str(player.hp) + " elupunkti.")
    elif tegevus.lower() in ["raha", "kuld", "kulda"]:
        kiiranimeeri("Teil on " + str(player.gold) + " kuldmünti.")
